...
克拉默法则  
设有行列式 $A_{nn}$ ，及其对应的矩阵 $\bold{A}$ 和方程组  
$\bold{Ax}=\bold{b}$  
如果 $A_{nn}\not =0$ ，则方程组有唯一解 $\displaystyle x_i=\frac{A_i}{A}$  
其中 $A_i$ 为将 $\bold{b}$ 替换掉 $A$ 中第 $i$ 列后形成的行列式。  
证：  
$\bold{Ax}=\bold{b}$ ，若 $|\bold{A}|\not =0$ ，则存在逆矩阵 $\bold{A}^{-1}$ ，即有 $\displaystyle \bold{x}=\bold{A}^{-1}\bold{b}=\frac{1}{|\bold{A}|}\bold{A}^*\bold{b}$  
$\displaystyle x_i=\sum_{k=1}^{n}\frac{1}{|\bold{A}|}A_{ki}b_k$ ，即得证。  
...
$\bold{A}=\begin{pmatrix}a_{11}&...&a_{1n}\\...&...&...\\a_{m1}&...&a_{mn}\end{pmatrix}$  
注意 $m\times n$ 的前后顺序，行数在前，列数在后，或者说是高度在前，宽度在后。  

$n$ 阶矩阵  
行数与列数都等于 $n$ 的矩阵称为 $n$ 阶矩阵或 $n$ 阶方阵，记作 $\bold{A}_n$  
