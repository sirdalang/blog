...
基变换公式  
设 $\Reals^n$ 有两组基 $(\boldsymbol{a}_i)$ ，$(\boldsymbol{b}_i)$ ，用 一组基表示另一组基的表示式就是基变换公式。  
求法：  
...
注：高等数学中，向量的投影是一个数。  
计算：记投影向量为 $\boldsymbol{c}$ ，则 $\displaystyle \boldsymbol{c}=||\boldsymbol{b}||\cos\theta \frac{\boldsymbol{a}}{||\boldsymbol{a}||}=\frac{[\boldsymbol{a},\boldsymbol{b}]}{||\boldsymbol{a}||^2}\boldsymbol{a}=\frac{[\boldsymbol{a},\boldsymbol{b}]}{[\boldsymbol{a},\boldsymbol{a}]}\boldsymbol{a}$  
...
施密特规范正交化  
先根据给定基求正交基：  
以三维空间为例，选定第一个向量，作为第一个结果向量；选定第二个向量，其与第一个结果向量形成的平面中，
